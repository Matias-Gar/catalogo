  ...
  useEffect(() => {
    const obtenerPacks = async () => {
      try {
        ...
        const now = new Date();
        const packsVigentes = (data || []).filter((pack) =>
          (!pack.fecha_fin || new Date(pack.fecha_fin) >= now) &&
          Array.isArray(pack.pack_productos) &&
          pack.pack_productos.every((item) => !item.productos?.archivado)
        );
        setPacks(packsVigentes);
      } catch (err) {
