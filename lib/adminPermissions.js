...
const ADMINISTRACION_PATHS = Array.from(new Set([...VENDEDOR_PATHS, ...ALMACEN_PATHS, '/admin/productos/editar', '/admin/productos/reducir-stock']));
...
export const ADMIN_MENU = [
  ...
  },
  {
    ...
    children: [
      ...
      { label: 'Reducir Stock', path: '/admin/productos/reducir-stock', roles: ['admin', 'administracion'] },
      { label: 'Transferencia Sucursal', path: '/admin/productos/transferencia-sucursal', roles: ['admin', 'administracion', 'almacen'] },
      { label: 'Editar Articulos', path: '/admin/productos/editar', roles: ['admin', 'administracion'] },
      { label: 'Eliminar Articulos', path: '/admin/productos/eliminar', roles: ['admin'] },
      { label: 'Archivar Productos', path: '/admin/productos/archivar', roles: ['admin'] },
    ],
  },
  {
  ...
  },
  {
    ...
    children: [
      ...
      { label: 'Tipos de producto', path: '/admin/configuracion/tipos-producto', roles: ['admin'] },
      { label: 'Cambiar de sección', path: '/admin/configuracion/cambiar-seccion', roles: ['admin'] },
      { label: 'Cambiar de categoría', path: '/admin/configuracion/cambiar-categoria', roles: ['admin'] },
      { label: 'Generador de QR', path: '/admin/configuracion/generador-qr', roles: ['admin'] },
    ],
  },
  {
    label: 'Herramientas de Imagen',
    path: '/admin/configuracion/herramientas-imagen',
    roles: ['admin'],
  },
];
...
export function getHighestAdminRole(roles = []) {
  return roles
    .map(normalizeAdminRole)
    .filter(Boolean)
    .reduce((bestRole, role) => {
      const bestPriority = ROLE_PRIORITY[bestRole] ?? 0;
      const rolePriority = ROLE_PRIORITY[role] ?? 0;
      return rolePriority > bestPriority ? role : bestRole;
    }, 'cliente');
}

export function getEffectiveAdminRole(globalRole, countryRoles = []) {
  return getHighestAdminRole([globalRole, ...countryRoles]);
}
