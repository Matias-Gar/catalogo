...
import { supabase } from "../lib/SupabaseClient"; // 👈 ajusté la ruta, porque ahora está en la raíz
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {productos.map((p) => (
          <div key={p.id} className="bg-white rounded-xl shadow p-4 text-center">
            ...
            <a
              href={`https://wa.me/${telefono}?text=Hola%20quiero%20el%20producto%20${encodeURIComponent(
                p.nombre
              )}`}
              target="_blank"
