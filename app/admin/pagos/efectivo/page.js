...
export default function PagosEfectivoPage() {
  ...
  useEffect(() => {
    async function fetchStats() {
      ...
      try {
        ...
        const pagosSistema = (summary.sales || []).filter(s => s.modo_pago && (s.modo_pago.toLowerCase() === "efectivo" || s.modo_pago.toLowerCase() === "cash"));
        const pagosManual = (summary.movements || []).filter(m => m.payment_method === "cash" && m.type === "income");
        ...
        // Listado de los últimos 20 pagos efectivo (sistema y manual)
        const pagosRecientes = [
          ...
          ...pagosManual.map(m => ({
            ...
            monto: m.amount,
            ref: m.id,
            descripcion: m.description || "Ingreso manual efectivo",
          })),
        ].sort((a, b) => new Date(b.fecha) - new Date(a.fecha)).slice(0, 20);
        setRows(pagosRecientes);
      } catch (err) {
        setError("Error cargando estadísticas de pagos efectivo");
      } finally {
        setLoading(false);
      }
    }
  ...
  return (
    <div style={{ maxWidth: 700, margin: "0 auto", padding: 24 }}>
      <Card>
        <CardHeader>
          <CardTitle>Estadísticas de Pagos en Efectivo</CardTitle>
          <CardDescription>Pagos recibidos en efectivo, discriminando entre sistema y manual (últimos 30 días)</CardDescription>
        </CardHeader>
        <CardContent>
          {loading ? (
            <div>Cargando...</div>
            ...
            <>
              ...
              <div style={{ marginBottom: 12, fontWeight: 500 }}>Pagos recientes (Efectivo):</div>
