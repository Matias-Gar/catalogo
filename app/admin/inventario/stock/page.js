...
export default function StockPage() {
  ...
  function getStockColor(stock) {
    // returns color classes for badge (keeps text contrast for accessibility)
    ...
    <div className="w-full min-h-screen flex flex-col items-center justify-start py-10 px-4 sm:px-6 lg:px-8 bg-gradient-to-b from-white via-slate-50 to-gray-100">
      <div className="w-full max-w-6xl">
        <header className="mb-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            <h1 className="text-3xl sm:text-4xl font-extrabold bg-clip-text text-transparent bg-gradient-to-r from-sky-600 to-indigo-600">Inventario — Stock</h1>
            <p className="mt-2 text-sm text-gray-600">Vista clara y responsiva del stock — revisa rápidamente productos con bajo inventario.</p>
          </div>
          ...
          <div className="flex items-center gap-3">
            ...
            <a href="/admin/productos/catalogo" className="inline-flex items-center gap-2 bg-indigo-600 hover:bg-indigo-700 text-white px-4 py-2 rounded-md shadow-md text-sm font-semibold">
              Ver Catálogo
            </a>
      ...
      {loading ? (
        <div className="w-full max-w-6xl mx-auto py-12 flex items-center justify-center">
          <div className="animate-pulse bg-sky-100 text-sky-700 px-6 py-4 rounded-lg shadow">Cargando productos...</div>
        </div>
      ) : productos.length === 0 ? (
        <div className="w-full max-w-6xl mx-auto py-12 text-center">
          <div className="inline-block bg-white shadow-md rounded-lg p-6">
            ...
            <p className="text-sm text-gray-500 mt-1">Agrega productos desde el panel para que aparezcan aquí.</p>
          </div>
        </div>
      ) : (
              ...
              <tbody className="divide-y">
                {productos.map((prod, idx) => (
                  <tr key={prod.user_id ?? prod.id ?? `${prod.nombre ?? 'producto'}-${idx}`} className="hover:bg-gray-50 transition">
                    ...
                    <td className="p-3 text-slate-600">{prod.descripcion}</td>
                    ...
                    <td className="p-3 text-center">
                      <span className={"inline-flex items-center px-3 py-1 rounded-full text-sm font-semibold " + getStockColor(Number(prod.stock))}>{prod.stock}</span>
                    </td>
                    <td className="p-3 text-center text-slate-500">{prod.codigo_barra}</td>
                    <td className="p-3 text-center">
                      <button onClick={() => sendWhatsappAlerta(prod)} className="inline-flex items-center gap-2 bg-amber-500 hover:bg-amber-600 text-white px-3 py-1 rounded-md text-sm font-semibold">Enviar alerta</button>
                    </td>
                  </tr>
                ))}
              </tbody>
                ...
                <div className="flex-1">
                  <div className="flex items-start justify-between">
                    <div>
                      ...
                      <div className="text-sm text-slate-500 mt-1">{prod.descripcion}</div>
                    </div>
                  ...
                  <div className="mt-3 flex items-center justify-between">
                    ...
                    <div>
                      <button onClick={() => sendWhatsappAlerta(prod)} className="inline-flex items-center gap-2 bg-amber-500 hover:bg-amber-600 text-white px-3 py-1 rounded-md text-sm font-semibold">Alerta</button>
                    </div>
                  </div>
                </div>
              </div>
