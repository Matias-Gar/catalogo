...
// --------------------------------------------------------------------------
// COMPONENTE 1: Modal de Confirmación
// --------------------------------------------------------------------------
...
// --------------------------------------------------------------------------
// CORRECCIÓN 1: Nuevo Componente de Modal de Vista Previa de Imagen
// --------------------------------------------------------------------------
function ImagePreviewModal({ isOpen, onClose, imageUrl, productName }) {
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-90 flex items-center justify-center p-4 z-50" onClick={onClose}>
      {/* El div interno usa e.stopPropagation() para que el clic dentro del modal no lo cierre */}
...
export default function AdminProductosPage() {
    ...
    // Asumimos 'admin' para que la página funcione.
    ...
    // --------------------------------------------------------------------------
    // CORRECCIÓN 2: NUEVOS ESTADOS PARA VISTA PREVIA DE IMAGEN
    // --------------------------------------------------------------------------
    ...
    // --------------------------------------------------------------------------
    // CORRECCIÓN 3: NUEVAS FUNCIONES PARA VISTA PREVIA DE IMAGEN
    // --------------------------------------------------------------------------
    const openImageModal = (imageUrl, productName) => {
        setCurrentImageUrl(imageUrl);
        setCurrentImageName(productName);
    ...
    // Función mejorada para cargar productos y su categoría
    const fetchProductos = async () => {
        ...
        if (error) {
            ...
            // Mapeamos los datos para obtener el nombre de la categoría
            const formattedData = data.map(p => ({
                ...p,
                // Accedemos a p.categorias y luego a p.categorias.nombre
    ...
    const handleAñadirProducto = async (e) => {
        ...
        try {
            ...
            // Aseguramos que category_id sea un número o null, no una cadena vacía
    ...
    return (
        <div className="min-h-screen bg-gray-50 p-6 sm:p-10">
            <header className="flex justify-between items-center mb-10 border-b pb-4">
                ...
                <Link href="/" className="px-4 py-2 bg-indigo-600 text-white rounded-lg shadow-md hover:bg-indigo-700 transition">
                    🏠 Ir a la Tienda
                </Link>
            </header>
            ...
            {message && (
                <div className={`p-4 mb-6 rounded-lg font-medium shadow-md ${message.startsWith('❌') ? 'bg-red-100 text-red-700' : 'bg-green-100 text-green-700'}`}>
                    {message}
                </div>
            )}
            ...
            {productos.length > 0 ? (
                <div className="overflow-x-auto bg-white rounded-xl shadow-lg">
                    ...
                    {/* CORRECCIÓN DE HIDRATACIÓN: NO DEBE HABER SALTOS DE LÍNEA O ESPACIOS ENTRE ETIQUETAS DE LA TABLA */}
                    <table className="min-w-full divide-y divide-gray-200"><thead>
                        ...
                        {productos.map((producto) => (
                            <tr key={producto.user_id} className="hover:bg-gray-50 transition duration-150">
                                ...
                                {/* -------------------------------------------------------------------------- */}
                                {/* CORRECCIÓN 4: Modificar la Celda de Imagen en la Tabla (Añade onClick) */}
                                {/* -------------------------------------------------------------------------- */}
                                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                    <img
                                        src={producto.imagen_url || "https://placehold.co/50x50/374151/FFFFFF?text=No"}
                                        alt={producto.nombre}
                                        // CLASES AGREGADAS PARA INDICAR QUE ES CLICKABLE
                                        className="h-12 w-12 object-cover rounded-md cursor-pointer hover:shadow-lg transition"
                                        // NUEVO HANDLER DE CLIC
                                        onClick={() => openImageModal(producto.imagen_url, producto.nombre)}
            ...
            {/* -------------------------------------------------------------------------- */}
            {/* CORRECCIÓN 5: Renderizar el Modal de Vista Previa de Imagen */}
            {/* -------------------------------------------------------------------------- */}
            <ImagePreviewModal
                isOpen={showImageModal}
                onClose={closeImageModal}
                imageUrl={currentImageUrl}
                productName={currentImageName}
            />
            {/* -------------------------------------------------------------------------- */}
